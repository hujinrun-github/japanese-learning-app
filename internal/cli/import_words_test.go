...
func TestImportWords_Success(t *testing.T) {
	...
	n, err := cli.ImportWordsFromFile(db, filePath, false)
	if err != nil {
...
func TestImportWords_InvalidJSON(t *testing.T) {
	...
	_, err := cli.ImportWordsFromFile(db, f.Name(), false)
	if err == nil {
...
func TestImportWords_FileNotFound(t *testing.T) {
	db := openTestDB(t)
	_, err := cli.ImportWordsFromFile(db, filepath.Join(os.TempDir(), "nonexistent_words_file.json"), false)
	if err == nil {
...
func TestImportWords_IdempotentOnDuplicate(t *testing.T) {
	...
	n1, err := cli.ImportWordsFromFile(db, filePath, false)
	if err != nil {
		t.Fatalf("first import error: %v", err)
	}

	// Second import of same file should not error and should not double-insert.
	n2, err := cli.ImportWordsFromFile(db, filePath, false)
	if err != nil {
